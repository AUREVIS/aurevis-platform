...
export default function CartPage() {
  ...
  const hasDailyGiftAccess = isHoReCaAccount || Boolean(profile?.daily_gift_access);
  ...
  async function submitOrder(event) {
    ...
    const { data, error: orderError } = await supabase.rpc("create_aurevis_order", {
      ...
      cart_items: items.map((item) => ({
        ...
        whatsapp_number: whatsappOptIn ? form.phone.trim() : "",
      ...
      try {
        await fetch("/.netlify/functions/telegram-order", {
          ...
          body: JSON.stringify({
            ...
            whatsapp: whatsappOptIn ? form.phone.trim() : "",
            whatsappOptIn,
          ...
          <form className="checkout-card" onSubmit={submitOrder}>
            ...
            <label>
              ...
              <input
                ...
                placeholder="+374…"
              />
              {profile?.phone && <small className="checkout-phone-hint">{t("registeredPhone")}</small>}
            </label>
